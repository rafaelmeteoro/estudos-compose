...
import com.meteoro.jetnews.ui.state.copyWithResult
 ...
 * the suspending method that produces a single value. The result of this call be returned
 ...
 * ```
 *
 * Repeat calls to onRefresh are conflated while a request is in progress.
 *
...
fun <Producer, T> produceUiState(
    ...
    val result = produceState(UiState<T>(loading = true), producer, key) {
        ...
        // launch a new coroutine to handle errorClear events sync
        launch {
